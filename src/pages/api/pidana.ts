...
const POST_REQUEST = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    INSERT INTO PIDANA
    VALUES
        ('${body.No_Perkara}',  '${body.Tuntutan}', '${body.Dakwaan}')`)
...
const DELETE_REQUEST = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    await executeQuery(`DELETE FROM PIDANA WHERE No_Perkara = '${req.body.key}' `)
...
const PATCH_REQUEST = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    UPDATE PIDANA
    SET
        Tuntutan = '${body.new.Tuntutan}',
        Dakwaan = '${body.new.Dakwaan}'
    WHERE No_Perkara = '${body.original.No_Perkara}'
    `)
