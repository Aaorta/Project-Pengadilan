...
const DELETE_REQUEST = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    await executeQuery(`DELETE FROM MENJAKSAI WHERE NIP = '${req.body.key}' `)
...
const PATCH_REQUEST = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    SET
        NIP = '${body.new.NIP}'
    WHERE No_Perkara = '${body.original.No_Perkara}' AND NIP = '${body.original.NIP}'
    `)
